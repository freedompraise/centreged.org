...
const Navbar = () => {
  ...
  const triggerStyle = cn(
    "group inline-flex h-10 w-max items-center justify-center rounded-md px-4 py-2 text-white hover:text-gold transition-colors focus:outline-none data-[state=open]:bg-primary/50"
  )
        ...
        link.children ? (
          <div key={link.name} className="text-white">
            ...
            >
              <span>{link.name}</span>
              <ChevronDown
                className={`transform transition-transform ${expandedSection === link.name ? 'rotate-180' : ''}`}
                size={20}
              />
            </button>
                        ...
                        <NavigationMenuContent>
                          <ul className="grid w-[400px] gap-3 p-4 md:w-[500px] md:grid-cols-2 lg:w-[600px]">
                            {link.children.map((child, idx) => (
                              <li key={child.name} className={idx === 0 ? "row-span-3" : ""}>
